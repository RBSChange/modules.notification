...
class notification_NotificationService extends f_persistentdocument_DocumentService
{
	...
	public function send($notification, $recipients, $replacementArray, $senderModuleName, $replyTo = null, $overrideSenderEmail = null)
	{
		...
		else
		{
			...
			$attributes['header'] = f_util_StringUtils::parseHtml($header);
			$attributes['footer'] = f_util_StringUtils::parseHtml($footer);
			$attributes['body'] = f_util_StringUtils::parseHtml($body);
			...
			{
				Framework::warn(__METHOD__.": no plain text template found.");
				$textBody = '';
			}
